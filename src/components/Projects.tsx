        ...
        <div className="flex flex-col justify-center items-center gap-10 mt-30">
            ...
            <div className="flex gap-10 items-center | max-sm:flex-col ">
                {projects.map(project=>(
                    <div key={project.id} className="flex flex-col w-110 h-110 max-sm:w-60 max-sm:h-60 items-center text-white gap-3 rounded-2xl bg-gray-700/30  hover:scale-105 transition-all duration-200 ease-out shadow-sm  hover:bg-gray-700/50">
                        <img src={project.imageUrl} alt={project.title} className="rounded-t-2xl overflow-hidden h-1/2 max-sm:hidden"/>
                        <div className="flex justify-around w-full pt-2">
                            {project.tools.map((tool,idx)=>(
                                <div key={idx} className="bg-gray-500/40 rounded-2xl px-2 | max-sm:hidden">{tool}</div>
                            ))}
                        </div>
                        <div className="text-xl font-semibold pt-3">{project.title}</div>
                        <div className="w-full text-center text-gray-300 px-5 | max-sm:text-sm">{project.description}</div>
